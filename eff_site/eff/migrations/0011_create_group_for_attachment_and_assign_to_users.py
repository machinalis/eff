...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        group_attachment = orm['auth.group']()
        group_attachment.name = 'attachments'
        group_attachment.save()

        # Get permissions
        delete_attachment = orm['auth.permission'].objects.get(
                                                codename=u'delete_attachment')
        add_attachment = orm['auth.permission'].objects.get(
                                                codename=u'add_attachment')
        delete_f_attachment = orm['auth.permission'].objects.get(
                                        codename=u'delete_foreign_attachments')
        # Assign perms to group
        group_attachment.permissions.add(delete_attachment, add_attachment,
            delete_f_attachment)

        # Assign group to users
        all_users = orm['auth.user'].objects.all()
        for user in all_users:
            user.groups.add(group_attachment)

    def backwards(self, orm):
        # Delete group_attachment of users
        group_attachment = orm['auth.group'].objects.get(name='attachments')
        all_users = orm['auth.user'].objects.all()
        for user in all_users:
            user.groups.remove(group_attachment)

        # Delete group attachment from the db
        group_attachment = orm['auth.group'].objects.get(name='attachments')
        group_attachment.delete()
    ...
    models = {
        ...
        'eff.billing': {
            'Meta': {'ordering': "['client', '-date']", 'object_name': 'Billing', '_ormbases': ['eff.CommercialDocumentBase']},
            'commercialdocumentbase_ptr': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['eff.CommercialDocumentBase']", 'unique': 'True', 'primary_key': 'True'}),
        ...
        'eff.commercialdocumentbase': {
            'Meta': {'ordering': "['client', '-date']", 'object_name': 'CommercialDocumentBase'},
        ...
        'eff.creditnote': {
            'Meta': {'ordering': "['client', '-date']", 'object_name': 'CreditNote', '_ormbases': ['eff.CommercialDocumentBase']},
            'commercialdocumentbase_ptr': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['eff.CommercialDocumentBase']", 'unique': 'True', 'primary_key': 'True'})
        },
        'eff.currency': {
        ...
        'eff.payment': {
            'Meta': {'ordering': "['client', '-date']", 'object_name': 'Payment', '_ormbases': ['eff.CommercialDocumentBase']},
            'commercialdocumentbase_ptr': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['eff.CommercialDocumentBase']", 'unique': 'True', 'primary_key': 'True'}),
        ...
        'eff.project': {
            ...
            'name': ('django.db.models.fields.CharField', [], {'max_length': '200'}),
            'repository': ('django.db.models.fields.URLField', [], {'default': "''", 'max_length': '200', 'blank': 'True'}),
